...
function JapaPage() {
  const [count, setCount] = useState(0);
    ...
    // Create refs for the elements to be highlighted
    ...
    // This function runs when the tour is finished
    ...
    const handleSave = async () => {
        ...
        // Guard: ensure there's a count to save
        if (count <= 0) {
        ...
        try {
            ...
            if (res.data && res.data.ok) {
                ...
                setCount(0); // Reset count on successful save
            } else {
  ...
  return (
    <>
     ...
     {isTourOpen && <JapaTour onComplete={handleTourComplete} elements={tourElements} />}
            ...
            {/* MODIFICATION: Add the ref to the main container */}
     ...
     <button
  onClick={() => window.history.back()} // A simple way to go back
...
>
  <ArrowLeft className="w-6 h-6 text-amber-700 group-hover:-translate-x-1 transition-transform duration-300" />
      ...
      {/* Background animations (no changes) */}
      ...
      {/* MODIFICATION 1: Added pb-24 (padding-bottom) for mobile to make space for the fixed buttons,
        and md:pb-8 to revert to original padding on desktop.
      */}
      <div className="relative z-10 flex flex-col items-center justify-center gap-8 p-8 pb-24 md:pb-8">
        {/* Central Counter Button (no changes) */}
        ...
        {/* Control Buttons */}
        {/*
          MODIFICATION 2: Changed classes to position this container.
          - Mobile (default): fixed to the bottom, full-width, with a background.
          - Desktop (md:): Reverts to relative positioning with its original margin-top.
        */}
        <div className="fixed lg:bottom-0 bottom-2 left-0 z-20 w-full flex justify-evenly items-center p-12   md:relative md:z-auto md:w-auto md:border-none md:bg-transparent md:backdrop-blur-none md:mt-28 md:p-0 md:gap-6">
          ...
          >
            ...
            <div className="relative flex items-center gap-3">
              ...
              {/* MODIFICATION 3: Removed mt-4 for proper vertical alignment */}
          ...
          <button
           ref={saveBtnRef}
            onClick={handleSave}
            className="group relative px-8  bg-gradient-to-r from-orange-500 to-rose-500 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 active:scale-95 overflow-hidden"
          >
            ...
            <div className="relative flex items-center gap-3">
              ...
              {/* MODIFICATION 3: Removed mt-4 for proper vertical alignment */}
              <span className="font-semibold text-white mt-4">Save</span>
            </div>
          </button>
        </div>
