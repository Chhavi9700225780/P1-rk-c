...
const VerseOfTheDay = ({ id, desc, chapter, verse }) => {
  ...
  // Helper to find the correct translation based on language preference
  const getTranslation = () => {
    if (!desc || desc.length === 0) return null;

    const translation = desc.find((item) => {
      if (DefaultLanguage === "hindi") {
        return item.language === "hindi" && item.author_name === "Swami Tejomayananda";
      }
      return item.language === "english" && item.author_name === "Swami Adidevananda";
    });

    return translation ? translation.description : null;
  };

  const translationText = getTranslation();
...
// Optimization: Memoize to prevent unnecessary re-renders from parent updates
